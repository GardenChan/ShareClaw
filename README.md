...
多人共养一只虾，最怕的就是"串味"——你的聊天记录被别人看到，别人的 AI 人格影响了你的。ShareClaw 支持**两级隔离**，确保每只虾都"互不串味"：

| 隔离级别 | 隔离范围 | 适用场景 | 配置复杂度 |
|---|---|---|--|
| 🛡️ **弱隔离（Session 级）** | 多微信号间**会话上下文独立**，共享记忆与人格 | 共养一只虾，能力共用 | 简单 |
| 🔒 **强隔离（Agent 级）** | 多微信号间**会话、记忆、人格、工作区全部独立** | 需要完全独立 AI 人格的场景 | 中等 |

> 📖 完整的隔离机制分析和配置指南，请参阅 **[OpenClaw 多微信号隔离性指南](./docs/openclaw-isolation-guide.md)**
...
| 🔒 **两级隔离** | 弱隔离（Session 级）到强隔离（Agent 级），按需选择，互不串味 |
...
### 架构二：分离部署 — 单节点（Standalone Remote）
...
**适用场景**：ShareClaw 部署在公网可访问的服务器上，远程管理一台独立的 OpenClaw 服务器。

### 架构三：分离部署 — 多节点集群（Remote Cluster）

ShareClaw 统一调度多台 OpenClaw 服务器，自动选择最空闲的节点。

```
  ┌──────────────────┐
  │                  │          ┌──────────────────────┐
  │  ShareClaw       │ ───────▶│  OpenClaw 节点 A       │ 队列: 3/6
  │  (Web + API)     │ │       │  openclaw-weixin      │
  │                  │ │       └──────────────────────┘
  │  ┌────────────┐  │ │
  │  │ 调度器      │  │ │       ┌──────────────────────┐
  │  │ 选最空闲    │──┘ ├─────▶│  OpenClaw 节点 B       │ 队列: 1/6 ← 选中
  │  └────────────┘    │       │  openclaw-weixin      │
  │                    │       └──────────────────────┘
  │                    │
  │                    │       ┌──────────────────────┐
  │                    └─────▶│  OpenClaw 节点 C       │ 队列: 5/6
  └──────────────────┘         │  openclaw-weixin      │
                               └──────────────────────┘
```

**调度策略**：查询所有节点的队列长度 → 选最短的 → 相同长度随机选 → 不健康的永久加入黑名单。

**适用场景**：团队或社区共享，需要统一调度多台 OpenClaw 服务器的微信坐席资源。
...
多个微信号同时挂载在同一个 OpenClaw 实例上时，ShareClaw 支持两级隔离：

- **弱隔离（Session 级）**：通过 `session.dmScope` 配置，实现多微信号间的会话上下文隔离，一行配置即可
- **强隔离（Agent 级）**：为每个微信号创建独立 Agent + bindings 路由，实现会话、记忆、人格、工作区的完全物理隔离

详细的隔离机制分析与配置指南，请参阅 **[OpenClaw 多微信号隔离性指南](./docs/openclaw-isolation-guide.md)**。
